...
## Visual-fidelity rebuild (software-engineer, 2026-07-03, follow-up pass)

The first landing implementation above shipped correct data/plumbing but generic Tailwind markup — it never had access to the actual design source (`variant-modern.jsx`, `variant-modern.css`, `styles.css`, `shared.jsx`, `event-modal.jsx`) and only inferred structure from field names. This pass rebuilds `components/landing/*` to visually match that design source, reimplemented as Next.js/TypeScript components (no CDN/Babel runtime):

- **Delivered:** full color system + fonts (Space Grotesk / Inter Tight / Cormorant Garamond, added to the existing `app/globals.css` `@import` — this project doesn't use `next/font`, so the addition follows the existing convention instead of introducing a new one); animated hex-grid SVG background (deterministic, seeded — not `Math.random()` — to avoid SSR/CSR hydration mismatches); sticky/blurred nav with mobile hamburger + slide-down panel + floating ES/EN toggle; hero with glow/outline headline, badges, stat grid, and an interactive D20 "roll for an event" widget (animates through faces, natural-20 triggers an inline special state); infinite ticker marquee; about section; a reusable `MarqueeRow` carousel (auto-scroll + mouse-drag + native touch scroll + keyboard arrows + prev/next buttons) used for events, past events, games and partners; a rebuilt event-details modal (hero image + overlay, meta panel, actions); 4-column footer; custom cursor (dot + trailing ring, disabled on touch).
- **Adapted (schema unchanged, per instruction):** the design's modal shows separate when/where/who/price/organizer rows plus a distinct bullet list — the `events` table has no dedicated columns for those beyond date, so (as already implemented in the prior pass) that detail stays folded into `description_es/en` as formatted text; the modal only breaks out "when" (from `date`/`end_date`) and "where" (the fixed club address) as real structured rows. A `getClubEventTone()` helper deterministically derives the design's warm/amber/ruby/olive card theming from each event's id, since there's no stored category/tone column.
- **Explicitly deferred, flagged rather than silently dropped (nice-to-have priority per the task):** the full Konami-code / shake-to-trigger / 5-tap-logo easter-egg suite, the roaming "catch the meeple" discount-code hunt, and the full-screen natural-20 modal + falling-dice canvas animation. Only the minimal natural-20 inline state inside the D20 hero widget was implemented (it's intrinsic to that widget). None of this needs new backend surface if implemented later — it was scoped out purely for time, not for any technical blocker.
- **Known content gap:** the design references photographic assets (hero background, "about" section photo, CTA character illustration, nav/footer logo image) that were never supplied as real URLs anywhere in this pipeline. Rather than fabricate `alealaspalmas.es` paths that might not exist, these render as themed gradient placeholders (about/CTA) or are simply omitted (hero bg image, nav/footer logo uses the existing Sword icon already used elsewhere in this app). Real asset URLs should be swapped in when available.
