...
- [ ] The `profiles.password_hash` column exists in the target Drizzle schema (`lib/db/schema/profiles.ts`) and its type/nullability matches what `lib/authjs/credentials-user.ts` expects. **As of this writing, PR #169's schema does not yet define this column while PR #170's Credentials provider already reads it — this is a blocking gap that must be resolved (in PR #169 or a follow-up) before step 2.3 below can succeed.** Verify this precondition by confirming the column is present in the merged schema (e.g. `grep passwordHash lib/db/schema/profiles.ts` or the generated migration SQL) before scheduling a real cutover.
