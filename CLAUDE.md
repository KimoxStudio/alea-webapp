...
- Every read of `reservations` / `saved_games` for a `member` session MUST filter `WHERE user_id = session.id`; member-scoped reads must also pass through a scoping guard as defense-in-depth after the DB fetch and before mapping rows to the public shape (admins exempt). Two non-duplicate guards exist: `assertMemberRowsScoped()` (`lib/server/data-scoping.ts`) guards the two member-facing list reads (`listVisibleReservations`, `listSavedGamesForSession`). `assertMemberRowsScopedSql()` (`lib/server/authz.ts`) is generic over any raw-SQL row shape and guards `activateReservationByTable()` — see `__tests__/server/member-row-scoping-enforcement.test.ts` (#389) for the automated check that a function taking `SessionUser` and reading either table always calls one of them (with an explicit, reasoned allowlist for the rare function that reads a single already-owned row by id and checks ownership inline instead).
