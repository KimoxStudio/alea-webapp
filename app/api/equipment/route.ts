...
export async function GET(request: NextRequest) {
  ...
  try {
    return auth.applyCookies(NextResponse.json(await listEquipment()))
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return admin.applyCookies(NextResponse.json(await createEquipment(admin.session, body), { status: 201 }))
  } catch (error) {
