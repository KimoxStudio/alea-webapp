...
import { updateEvent, deleteEvent } from '@/lib/server/events/events-service'
...
// with different validation than lib/server/events/club-events-service.ts. Do not
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return admin.applyCookies(NextResponse.json(await updateEvent(admin.session, id, body)))
  } catch (error) {
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await deleteEvent(admin.session, id)
