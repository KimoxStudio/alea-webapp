 ...
 * guards the two member-facing list reads in `saved-games-service.ts`
 * (`listSavedGamesForSession`) and `reservations-service.ts`
 * (`listVisibleReservations`). `assertMemberRowsScopedSql()` here guards
 * `activateReservationByTable()` (`reservations-service.ts`, #389) — its
 * first production call site — and remains the intended defense-in-depth
 * seam for the next raw-SQL member-scoped read (per the CLAUDE.md convention
 * this module's header documents). `users-service.ts` is admin-only (see its
 * own header comment) and has no member-scoped reads to guard. Both guards
 * are covered by their own tests plus `member-row-scoping-contract.ts`, and
 * `__tests__/server/member-row-scoping-enforcement.test.ts` (#389) fails CI
 * if a future `SessionUser`-scoped read of `reservations`/`saved_games` ships
 * without calling either one.
 */
