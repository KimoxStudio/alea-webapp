...
const MAGIC_BYTE_SIGNATURES: Record<ImageMime, (bytes: Uint8Array) => boolean> = {
  'image/png': (bytes) => startsWithSignature(bytes, [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]),
  'image/jpeg': (bytes) => startsWithSignature(bytes, [0xff, 0xd8, 0xff]),
  'image/webp': (bytes) =>
  ...
  'image/gif': (bytes) =>
    // "GIF87a" or "GIF89a"
    startsWithSignature(bytes, [0x47, 0x49, 0x46, 0x38]) &&
    bytes.length >= 6 &&
